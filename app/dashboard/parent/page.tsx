...
export default async function ParentDashboard() {
  ...
  const [{ data: attendances }, { data: grades }, { data: assignments }] = await Promise.all([
    childIds.length > 0
      ? supabase
          .from('attendance')
          .select('student_id, date, status, subjects(name), students(users(full_name))')
    ...
    childIds.length > 0
      ? supabase
          .from('grades')
          .select('score, student_id, created_at, exams(title, pass_score, subjects(name)), students(users(full_name))')
          .in('student_id', childIds)
    ...
    childIds.length > 0
      ? supabase
          ...
          .in(
            'section_id',
            children?.map(c => (c.sections as any)?.id).filter(Boolean) || []
          )
          .order('due_date')
          .limit(5)
      : Promise.resolve({ data: [] }),
  ...
  const avgScore =
    grades && grades.length > 0
      ? Math.round(
          (grades as any[]).reduce((a: number, g: any) => a + g.score, 0) /
            grades.length
        )
      : 0
    ...
    <div className="p-6 space-y-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-blue-200 text-sm">
                  {(child.sections as any)?.classes?.name} — شعبة{' '}
                  {(child.sections as any)?.name}
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-bold text-gray-800 flex items-center gap-2">
              <UserCheck className="w-5 h-5 text-blue-600" />آخر سجلات الحضور
            </h2>
            <Link href="/dashboard/parent/attendance" className="text-xs text-blue-600">
              عرض الكل
            </Link>
          </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400">
                    {(a.subjects as any)?.name} — {formatDate(a.date)}
                  </p>
                </div>
              </div>
            ))
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-bold text-gray-800 flex items-center gap-2">
              <BarChart3 className="w-5 h-5 text-purple-600" />آخر النتائج
            </h2>
            <Link href="/dashboard/parent/grades" className="text-xs text-blue-600">
              عرض الكل
            </Link>
          </div>
          ...
          {grades?.length === 0 ? (
            ...
            (grades as any[])?.map((g: any) => (
              <div
                key={`${g.student_id}-${g.created_at}`}
                className="flex items-center justify-between p-2.5 mb-1.5 hover:bg-gray-50 rounded-xl"
              >
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400">
                    {g.students?.users?.full_name} — {g.exams?.subjects?.name}
                  </p>
                </div>

                <span
                  className={`px-2.5 py-1 rounded-lg text-xs font-bold ${getScoreBg(
                    g.score
                  )}`}
                >
                  {Math.round(g.score)}%
