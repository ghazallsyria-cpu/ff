...
export default async function StudentGradesPage() {
  ...
  const { data: grades } = await supabase
    .from('grades')
    .select('*, exams(title, pass_score, subjects(name), sections(name, classes(name)))')
  ...
  const avg = grades && grades.length > 0
    ...
    : 0
  const passed = grades?.filter(g => g.score >= (g.exams as any)?.pass_score).length ?? 0

  return (
    <div className="p-6">
      <PageHeader title="درجاتي ونتائجي" subtitle="نتائج كل اختباراتي" />
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-blue-50 rounded-2xl p-4 text-center">
          <p className="text-3xl font-bold text-blue-600">{grades?.length ?? 0}</p>
          <p className="text-sm text-gray-500 mt-1">إجمالي الاختبارات</p>
        </div>
        ...
        <div className="bg-purple-50 rounded-2xl p-4 text-center">
          ...
          <p className="text-sm text-gray-500 mt-1">اختبارات ناجحة</p>
        </div>
      </div>
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        {grades?.length === 0
          ? <p className="text-center text-gray-400 py-10">لا توجد درجات مسجلة بعد</p>
          : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-gray-500 text-xs">
                <tr>
                  <th className="text-right px-4 py-3 font-semibold">الاختبار</th>
                  <th className="text-right px-4 py-3 font-semibold">المادة</th>
                  <th className="text-right px-4 py-3 font-semibold">الصف</th>
                  <th className="text-center px-4 py-3 font-semibold">الدرجة</th>
                  <th className="text-center px-4 py-3 font-semibold">الحالة</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {grades?.map(g => {
                  ...
                  const passed = g.score >= exam?.pass_score
                  return (
                    <tr key={g.id} className="hover:bg-gray-50/50">
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className={`px-2.5 py-1 rounded-lg text-xs font-semibold ${passed ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                          {passed ? 'ناجح' : 'راسب'}
                        </span>
                      </td>
