...
export default async function StudentDashboard() {
  ...
  const [
    ...
    supabase.from('exams')
      .select('id, title, start_at, subjects(name)')
    ...
    supabase.from('grades')
      .select('id, score, exams(title, subjects(name))')
    ...
    supabase.from('schedules')
      .select('id, period, start_time, subjects(name), teachers(users(full_name))')
      .eq('section_id', student?.section_id ?? '')
  ...
  return (
    <div className="p-6 space-y-6">
      <PageHeader
        ...
        subtitle={`${DAYS_AR[today]} — ${(student?.sections as any)?.classes?.name} شعبة ${(student?.sections as any)?.name}`}
      />
              ...
              <div key={s.id} className="flex items-center gap-3 p-2.5 mb-2 bg-blue-50 rounded-xl">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400">
                    {(s.teachers as any)?.users?.full_name}
                  </p>
                </div>
                {s.start_time && (
                  <span className="text-xs text-gray-400">{s.start_time}</span>
                )}
              </div>
            ))
          )}
              ...
              <div key={e.id} className="p-3 mb-2 border border-purple-100 rounded-xl bg-purple-50">
                ...
                {e.start_at && (
                  <p className="text-xs text-gray-400 mt-1">
                    يبدأ: {formatDate(e.start_at)}
                  </p>
                )}
                <Link
                  href={`/dashboard/student/exams/${e.id}`}
                  className="mt-2 inline-block text-xs text-white bg-purple-600 px-3 py-1 rounded-lg"
                >
                  دخول الاختبار
                </Link>
              </div>
          ...
          {myGrades?.length === 0 ? (
            ...
            myGrades?.map((g: any) => (
              <div key={g.id} className="flex items-center justify-between p-2.5 mb-2 hover:bg-gray-50 rounded-xl">
                <div>
                  <p className="font-medium text-gray-800 text-sm">
                    {(g.exams as any)?.title}
                  </p>
                  <p className="text-xs text-gray-400">
                    {(g.exams as any)?.subjects?.name}
                  </p>
                </div>
                <span className={`px-2.5 py-1 rounded-lg text-xs font-bold ${getScoreBg(g.score)}`}>
                  {Math.round(g.score)}%
                </span>
              </div>
            ))
