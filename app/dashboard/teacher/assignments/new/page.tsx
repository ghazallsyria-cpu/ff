...
export default function NewAssignmentPage() {
  ...
  const [form, setForm] = useState({
    title: '', description: '', section_id: '', subject_id: '', due_date: '',
  })
        ...
        <form onSubmit={handleSave} className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={form.section_id} onChange={e => setForm(f => ({ ...f, section_id: e.target.value, subject_id: '' }))} required
                className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                <option value="">اختر الشعبة</option>
                {[...new Map(sections.map(s => [s.section_id, s])).values()].map(s => (
                  <option key={s.section_id} value={s.section_id}>{s.sections?.classes?.name} - {s.sections?.name}</option>
                ))}
              </select>
            </div>
            <div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">وصف الواجب</label>
