...
import { ATTENDANCE_STATUS_AR, DAYS_AR } from '@/lib/utils'
...
export default function TeacherAttendancePage() {
  ...
  async function saveAttendance() {
    ...
    await supabase.from('attendance').upsert(records, { onConflict: 'student_id,subject_id,date,period' })
    setSaving(false)
    setSaved(true)
    setTimeout(() => setSaved(false), 2000)
  }

  const selectedTS = sections.find(s => s.section_id === selectedSection && s.subject_id === selectedSubject)
  ...
  return (
    <div className="p-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-5">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            ...
            <select value={selectedSection} onChange={e => { setSelectedSection(e.target.value); setSelectedSubject('') }}
          ...
          <div>
            ...
            <select value={selectedSubject} onChange={e => setSelectedSubject(e.target.value)} disabled={!selectedSection}
              ...
              {sections.filter(s => s.section_id === selectedSection).map(s => (
        ...
        <button onClick={loadStudents} disabled={!selectedSection || !selectedSubject}
      ...
      {loading && <div className="flex justify-center py-10"><Loader2 className="w-8 h-8 animate-spin text-blue-600" /></div>}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100">
          <div className="p-4 border-b border-gray-100 flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-gray-800">{students.length} طالب</span>
            </div>
            <div className="flex gap-2">
              {Object.entries(ATTENDANCE_STATUS_AR).map(([k, v]) => (
                <button key={k} onClick={() => markAll(k as AttendanceStatus)}
          ...
          <div className="divide-y divide-gray-50">
            {students.map((s, i) => (
              <div key={s.id} className="flex items-center gap-4 px-4 py-3 hover:bg-gray-50/50">
                <span className="text-sm text-gray-400 w-8 text-center font-mono">{i + 1}</span>
                <div className="flex-1">
                  <p className="font-medium text-gray-800 text-sm">{s.users?.full_name}</p>
                  ...
                  {(Object.entries(ATTENDANCE_STATUS_AR) as [AttendanceStatus, string][]).map(([k, v]) => (
                    <button key={k} onClick={() => setStatus(s.id, k)}
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                        s.status === k
                          ? k === 'present' ? 'bg-green-600 text-white' : k === 'absent' ? 'bg-red-600 text-white' : k === 'late' ? 'bg-yellow-500 text-white' : 'bg-blue-600 text-white'
          ...
          <div className="p-4 border-t border-gray-100">
            <button onClick={saveAttendance} disabled={saving}
              ...
              {saving ? <><Loader2 className="w-4 h-4 animate-spin" />جاري الحفظ...</>
               : saved ? <>✅ تم الحفظ</>
               : <><Save className="w-4 h-4" />حفظ الحضور</>}
            </button>
          </div>
        </div>
      )}
